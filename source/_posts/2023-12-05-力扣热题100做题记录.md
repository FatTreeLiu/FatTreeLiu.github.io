...
这版写的很清晰，注意先求当前位的数，再求进位值，否则进位值会影响当前位的值。
    ...
    public int orangesRotting(int[][] grid) {
        ...
        while(fresh > 0 && !queue.isEmpty()){
            ...
            for (int i = 0; i < size; i++){
                int[] bad = queue.remove();
                if (bad[0] - 1 >= 0 && grid[bad[0] - 1][bad[1]] == 1){
                    queue.add(new int[]{bad[0] - 1, bad[1]});
                    grid[bad[0] - 1][bad[1]] = 2;
                    fresh--;
                }
                if (bad[0] + 1 < grid.length && grid[bad[0] + 1][bad[1]] == 1){
                    queue.add(new int[]{bad[0] + 1, bad[1]});
                    grid[bad[0] + 1][bad[1]] = 2;
                    fresh--;
                }
                if (bad[1] - 1 >= 0 && grid[bad[0]][bad[1] - 1] == 1){
                    queue.add(new int[]{bad[0], bad[1] - 1});
                    grid[bad[0]][bad[1] - 1] = 2;
                    fresh--;
                }
                if (bad[1] + 1 < grid[0].length && grid[bad[0]][bad[1] + 1] == 1){
                    queue.add(new int[]{bad[0], bad[1] + 1});
                    grid[bad[0]][bad[1] + 1] = 2;
                    fresh--;
                }
...
取大于等于target的第一个数，如果nums[mid] < target，则mid绝不可能成为解，所以left=mid+1，如果nums[mid] >= target，mid可能成为解，所以right=mid，因为推出条件为right == left，所以返回任意一个都可以。
...
class Solution {
    public int[] dailyTemperatures(int[] temperatures) {
        ...
        for (int i = 0; i < temperatures.length; i++){
            if (stack.isEmpty() || stack.peekLast()[1] >= temperatures[i]){
...
基于二维dp进行遍历。dp[i][j]表示字符串从i到j是否是回文字符串。注意特殊情况为子字符串长度小于等于3的时候。这个题目技巧性较强，好好记忆。
